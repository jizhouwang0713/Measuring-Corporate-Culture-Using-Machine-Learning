...
model = gensim.models.Word2Vec.load(
    str(Path(global_options_qa.MODEL_FOLDER, "w2v", "w2v.mod"))
)
...
expanded_words = culture_dictionary.expand_words_dimension_mean(
    ...
    seed_words=global_options_qa.SEED_WORDS,
    restrict=global_options_qa.DICT_RESTRICT_VOCAB,
    n=global_options_qa.N_WORDS_DIM,
)
...
expanded_words = culture_dictionary.rank_by_sim(
    expanded_words, global_options_qa.SEED_WORDS, model
)
# output the dictionary
culture_dictionary.write_dict_to_csv(
    culture_dict=expanded_words,
    file_name=str(Path(global_options_qa.OUTPUT_FOLDER, "dict", "expanded_dict.csv")),
)
print("Dictionary saved at {}".format(str(Path(global_options_qa.OUTPUT_FOLDER, "dict", "expanded_dict.csv"))))
