...
id2firm = pd.read_csv(str(Path(global_options_qa.DATA_FOLDER, "input", "id2firms.csv")))

methods = ["TF", "TFIDF", "WFIDF"]
for method in methods:
    scores = pd.read_csv(
        str(
            Path(global_options_qa.OUTPUT_FOLDER, "scores", "scores_{}.csv".format(method))
        )
    )
    scores = scores.merge(
        id2firm, how="left", left_on=["Doc_ID"], right_on="document_id"
    ).drop(["Doc_ID", "document_id"], axis=1)
    for dim in global_options_qa.DIMS:
