...
def process_largefile(
    ...
    with open(input_file, newline="\n", encoding="utf-8", errors="ignore") as f_in:
        ...
        for next_n_lines, next_n_line_ids in zip(
            ...
            output_lines = []
            output_line_ids = []
            with Pool(global_options_qa.N_CORES) as pool:
...
if __name__ == "__main__":
    with CoreNLPClient(
        ...
        memory=global_options_qa.RAM_CORENLP,
        threads=global_options_qa.N_CORES,
        timeout=12000000,
        endpoint="http://localhost:9002",  # change port here and in preprocess_parallel.py if 9002 is occupied
        max_char_length=1000000,
    ) as client:
        in_file = Path(global_options_qa.DATA_FOLDER, "input", "documents.txt")
        in_file_index = file_util.file_to_list(
            Path(global_options_qa.DATA_FOLDER, "input", "document_ids.txt")
        )
        out_file = Path(
            global_options_qa.DATA_FOLDER, "processed", "parsed", "documents.txt"
        )
        ...
        process_largefile(
            ...
            chunk_size=global_options_qa.PARSE_CHUNK_SIZE,
        )
