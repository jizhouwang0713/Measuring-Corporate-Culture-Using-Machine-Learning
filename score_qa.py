...
def construct_doc_level_corpus(sent_corpus_file, sent_id_file):
    ...
    Dump "corpus_doc_level.pickle" and "doc_ids.pickle" to Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp"). 
    ...
    with open(
        Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp", "corpus_doc_level.pickle"),
        "wb",
    ) as out_f:
        pickle.dump(corpus, out_f)
    with open(
        Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp", "doc_ids.pickle"), "wb"
    ) as out_f:
...
def calculate_df(corpus):
    ...
    # save df dict
    with open(
        Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp", "doc_freq.pickle"), "wb"
    ) as f:
...
def load_doc_level_corpus():
    ...
    with open(
        Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp", "doc_ids.pickle"), "rb"
    ) as in_f:
...
def score_tf(documents, doc_ids, expanded_dict):
    ...
    score = culture_dictionary.score_tf(
        ...
        n_core=global_options_qa.N_CORES,
    )
    score.to_csv(
        Path(global_options_qa.OUTPUT_FOLDER, "scores", "scores_TF.csv"), index=False
    )
...
def score_tf_idf(documents, doc_ids, N_doc, method, expanded_dict, **kwargs):
    ...
    if method == "TF":
        ...
        df_dict = pd.read_pickle(
            Path(global_options_qa.OUTPUT_FOLDER, "scores", "temp", "doc_freq.pickle")
        )
        # score tf-idf
        ...
        score.to_csv(
            str(
                Path(
                    global_options_qa.OUTPUT_FOLDER,
                    "scores",
...
if __name__ == "__main__":
    current_dict_path = str(
        str(Path(global_options_qa.OUTPUT_FOLDER, "dict", "expanded_dict.csv"))
    )
    ...
    corpus, doc_ids, N_doc = construct_doc_level_corpus(
        ...
        sent_id_file=Path(
            global_options_qa.DATA_FOLDER, "processed", "parsed", "document_sent_ids.txt"
        ),
    )
